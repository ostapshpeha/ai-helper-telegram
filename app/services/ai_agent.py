...
from agentlightning import prompt_rollout
...
# Декоратор перетворює цю функцію на LitAgent, зрозумілий для фреймворку
@prompt_rollout
async def training_agent_wrapper(task: str, prompt_template: str) -> float:
    """
    Ця функція отримує завдання (task) і новий згенерований алгоритмом промпт.
    Вона має повернути float (нагороду від 0.0 до 1.0).
    """

    # 1. ТИМЧАСОВО змінюємо системний промпт нашого агента на той, що дав алгоритм
    # (PydanticAI дозволяє перевизначати промпти під час запуску)

    try:
        # Запускаємо твого агента з новим промптом
        result = await honda_agent.run(task, system_prompt=prompt_template)
        ...
        if response_text and len(response_text) > 20:
            score += 0.5
        # Verify Ukrainian (Cyrillic chars present)
        if any('\u0400' <= c <= '\u04FF' for c in response_text):
            score += 0.3
        # Penalise "I don't know" responses
        if "не знаю" not in response_text.lower() and "не можу відповісти" not in response_text.lower():
            score += 0.2

        # Повернення float автоматично створює фінальний "reward span" у базі [cite: 3103]
        return min(score, 1.0)

    except Exception as e:
        # Якщо бот зламався через поганий промпт
        return 0.0
